...
Sphere::Sphere(float rad, GLuint sl, GLuint st, ShaderProgram *shader) :
radius(rad), slices(sl), stacks(st), _shader(shader)
{
...
void Sphere::setup(float *vertices, float *normals, unsigned int *el, float *texture) 
{
	...
	//create vbos for texture 
	glGenBuffers(1, &VBO_texture);		//
	glBindBuffer(GL_ARRAY_BUFFER, VBO_texture);  //bind the vbo
	//create a buffer in GPU memory and copy the data
	glBufferData(GL_ARRAY_BUFFER, nVerts * 3 * sizeof(float), texture, GL_STATIC_DRAW);
	glVertexAttribPointer(
		2,				//attribute number
